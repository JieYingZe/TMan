...
$sql = "SELECT `question`.`questionid`, count( `answer`.`answerid` ) AS count_answer FROM `question` LEFT JOIN `answer` ON `question`.`questionid` = `answer`.`question_questionid` GROUP BY `question`.`questionid` ORDER BY `question`.`create_time` DESC";
